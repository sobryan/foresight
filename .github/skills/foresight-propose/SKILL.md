---
name: foresight-propose
description: Turn the top coverage gaps into ready-to-run regression proposals for /tdd
argument-hint: [N | gap-id]
---

Argument: the arguments the user typed after the command  (a count N — default 3 — or a specific gap id)

Generate new-regression **proposals** for the highest-value uncovered gaps:

1. Ensure coverage + audit are current (run `.github/skills/foresight/scripts/foresight.py coverage` and `audit` if `coverage/coverage.json` / `audit/audit.json` are missing or stale).
