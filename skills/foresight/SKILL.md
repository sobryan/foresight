...
The deterministic core lives at `${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py`. Run it directly for the mechanical steps; delegate to sub-agents for the reading/exploring/judgment steps. After any agent writes an artifact, `foresight.py validate` is cheap insurance that the next phase gets well-formed input.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" catalog --project "$(pwd)"
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" visual --project "$(pwd)" [--threshold PCT]
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" visual --project "$(pwd)" --baseline
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" audit --project "$(pwd)"          # add --fix-run-command when the user passed --fix
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" reorg --project "$(pwd)" --apply    # --force also overwrites protected values
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" validate --project "$(pwd)"
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/foresight.py" report --project "$(pwd)" --html
```
